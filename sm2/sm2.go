...
func GetRSFromSig(sig []byte) (*big.Int, *big.Int, error) {
	var (
		r, s  = &big.Int{}, &big.Int{}
		inner cryptobyte.String
	)
	...
	if !input.ReadASN1(&inner, cbasn1.SEQUENCE) ||
		...
		!inner.ReadASN1Integer(s) ||
		!inner.Empty() {
		return nil, nil, errors.New("failed to parse signature to get R and S")
	}
	return r, s, nil
}
...
func Sm2Hash(msg []byte, pub *PublicKey) (*big.Int, error) {
...
func ZA(pub *PublicKey, uid []byte) ([]byte, error) {
	...
	za.Write(sm2P256ToBig(&sm2P256.A_).Bytes())
...
func GenerateKey_(d *big.Int, curve elliptic.Curve) (*PrivateKey, error) {
	k := new(big.Int).Set(d)
	priv := new(PrivateKey)
	priv.D = k
